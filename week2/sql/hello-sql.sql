...
-- within each database, we have "schemas"
...
-- USE AdventureWorksLT would switch databases from master, but Azure SQL DB doesn't support it
-- so have to use the dropdown instead
...
-- setting for datetime format, number format

-- the SELECT statement (or command) ...
-- doesn't even need to access the db
SELECT 1;
SELECT CURRENT_TIMESTAMP;

-- we get all columns and all rows from a table with SELECT *.
-- SELECT *, and FROM.
SELECT * 
...
-- we can do "aliases", renaming columns or combining them, with AS
...
-- string are ordered by "dictionary order" "lexicographic order"
...
-- ORDER BY clause accomplished this.

-- all customers first name starting with C
-- ordered by first name, with last name (descending order) as tie breaker
SELECT * 
FROM SalesLT.Customer
WHERE FirstName >= 'c' AND FirstName < 'd'
...
-- we can do regex-lite pattern matching on string with LIKE operator.
